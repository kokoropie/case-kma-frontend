"use client";
import Phone from "@/components/Phone";
import { useState } from "react";
import Confetti from "react-dom-confetti";
import {
  Color,
  Finish,
  Material,
  Model,
  useGetConfiguration,
} from "../design/actions";
import { ArrowRight, Check, Loader2 } from "lucide-react";
import { formatPrice } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { useRouter } from "next/navigation";
import { useCreateMyConfiguration } from "./actions";

const DesignPreview = ({}) => {
  const router = useRouter();
  const [showConfetti, setShowConfetti] = useState<boolean>(false);
  const { configurationData, isLoadingConfig } = useGetConfiguration();
  const { createMyConfiguration, isLoading: isLoadingCreateConfig } =
    useCreateMyConfiguration();
  const configuration: {
    color: Color;
    model: Model;
    material: Material;
    finish: Finish;
  } = JSON.parse(localStorage.getItem("configuration") as string);

  const handleCheckout = async () => {
    // console.log({
    //   color: configuration.color.slug,
    //   finish: configuration.finish.slug,
    //   material: configuration.material.slug,
    //   model: configuration.model.slug,
    //   width: Number(localStorage.getItem("width")),
    //   height: Number(localStorage.getItem("height")),
    //   image: localStorage.getItem("image") as string,
    //   croppedImage: localStorage.getItem("croppedImage") as string,
    // });
    const res: any = await createMyConfiguration({
      color: configuration.color.slug,
      finish: configuration.finish.slug,
      material: configuration.material.slug,
      model: configuration.model.slug,
      width: Number(localStorage.getItem("width")),
      height: Number(localStorage.getItem("height")),
      image: localStorage.getItem("image") as string,
      croppedImage: localStorage.getItem("croppedImage") as string,
    });

    router.push(`/checkout?configuration_id=${res?.configuration_id}`);
  };

  if (isLoadingConfig) {
    return <span>Loading...</span>;
  }

  return (
    <>
      <div
        aria-hidden="true"
        className="pointer-events-none select-none absolute inset-0 overflow-hidden flex justify-center"
      >
        <Confetti
          active={showConfetti}
          config={{ elementCount: 200, spread: 90 }}
        />
      </div>

      <div className="mt-20 flex flex-col items-center md:grid text-sm sm:grid-cols-12 sm:grid-rows-1 sm:gap-x-6 md:gap-x-8 lg:gap-x-12">
        <div className="md:col-span-4 lg:col-span-3 md:row-span-2 md:row-end-2">
          <Phone
            style={{ backgroundColor: configuration.color.hex_code }}
            className={"max-w-[150px] md:max-w-full"}
            imgSrc={localStorage.getItem("croppedImage") as string}
          />
        </div>

        <div className="mt-6 sm:col-span-9 md:row-end-1">
          <h3 className="text-3xl font-bold tracking-tight text-gray-900">
            Your {configuration.model.name} Case
          </h3>
          <div className="mt-3 flex items-center gap-1.5 text-base">
            <Check className="h-4 w-4 text-green-500" />
            In stock and ready to ship
          </div>
        </div>

        <div className="sm:col-span-12 md:col-span-9 text-base">
          <div className="grid grid-cols-1 gap-y-8 border-b border-gray-200 py-8 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10">
            <div>
              <p className="font-medium text-zinc-950">Highlights</p>
              <ol className="mt-3 text-zinc-700 list-disc list-inside">
                <li>Wireless charging compatible</li>
                <li>TPU shock absorption</li>
                <li>Packaging made from recycled materials</li>
                <li>5 year print warranty</li>
              </ol>
            </div>
            <div>
              <p className="font-medium text-zinc-950">Materials</p>
              <ol className="mt-3 text-zinc-700 list-disc list-inside">
                <li>High-quality, durable material</li>
                <li>Scratch- and fingerprint resistant coating</li>
              </ol>
            </div>
          </div>

          <div className="mt-8">
            <div className="bg-gray-50 p-6 sm:rounded-lg sm:p-8">
              <div className="flow-root text-sm">
                <div className="flex items-center justify-between py-1 mt-2">
                  <p className="text-gray-600">Base price</p>
                  <p className="font-medium text-gray-900">
                    {formatPrice(Number(configurationData?.base))}
                  </p>
                </div>

                <div className="flex items-center justify-between py-1 mt-2">
                  <p className="text-gray-600">
                    <span className="capitalize">
                      {configuration.finish.name}
                    </span>{" "}
                    finish
                  </p>
                  <p className="font-medium text-gray-900">
                    {formatPrice(configuration.finish.price)}
                  </p>
                </div>

                <div className="flex items-center justify-between py-1 mt-2">
                  <p className="text-gray-600">
                    <span className="capitalize">
                      {configuration.material.name}
                    </span>{" "}
                    material
                  </p>
                  <p className="font-medium text-gray-900">
                    {formatPrice(configuration.material.price)}
                  </p>
                </div>

                <div className="my-2 h-px bg-gray-200" />

                <div className="flex items-center justify-between py-2">
                  <p className="font-semibold text-gray-900">Order total</p>
                  <p className="font-semibold text-gray-900">
                    {formatPrice(
                      Number(configurationData?.base) +
                        configuration.material.price +
                        configuration.finish.price
                    )}
                  </p>
                </div>
              </div>
            </div>

            <div className="mt-8 flex justify-end pb-12">
              <Button
                onClick={() => handleCheckout()}
                className="px-4 sm:px-6 lg:px-8"
              >
                {isLoadingCreateConfig ? (
                  <Loader2 className="animate-spin" />
                ) : (
                  <>
                    Check out <ArrowRight className="h-4 w-4 ml-1.5 inline" />
                  </>
                )}
              </Button>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default DesignPreview;
